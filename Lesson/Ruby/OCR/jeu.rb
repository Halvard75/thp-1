...
class Joueur < Personne
 ...
 def soin
    ...
    if @points_de_vie == 100
      puts "#{nom} n'est pas blessé donc pas de se soigner"
    else
    soin = rand(20..40)
    @points_de_vie=[points_de_vie+soin,100].min
      puts "#{nom} regagne de la vie (#{soin}pv)."
    end
  end
...
class Monde
  ...
  def ennemis_en_vie
    ...
    ennemis_en_vie=[]
    @ennemis.each do |badass|
      if badass.points_de_vie >0
        ennemis_en_vie << badass
      end
    end
