...
def get_all_the_urls_of_department_townhalls (url_department)
		...
		url_page_list_final =[]
		url_page_list.each {|i| url_page_list_final << {:ville => i.text,:url => "http://annuaire-des-mairies.com/#{i["href"]}",:email => ""}}
		...
		CSV.open("data_mairies_emailXOVER.csv", "wb") do |csv|
		  csv << table_email_adress.first.keys # adds the attributes name on the first line
		  table_email_adress.each do |hash|
		    csv << hash.values
		  end
		end
		puts "Les adresses des mairies sont prêtes bro !"

end

#Test functions by getting all email-adresses from val-d'oise
get_all_the_urls_of_department_townhalls ("http://annuaire-des-mairies.com/haute-garonne.html")
#get_all_the_urls_of_department_townhalls ("http://annuaire-des-mairies.com/haute-garonne-2.html")
#get_all_the_urls_of_department_townhalls ("http://annuaire-des-mairies.com/haute-garonne-3.html")
